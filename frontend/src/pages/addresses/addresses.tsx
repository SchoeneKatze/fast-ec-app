...
export default function AddressManagement() {
    ...
    const fetchAddresses = useCallback(async () => {
        ...
        if (claims) {
            try {
                const response = await fetch(`${BACKEND_URL}/addresses/getAddresses?logto_id=${claims.sub}`);
                const data = await response.json();
                const addressArray = Array.isArray(data) ? data : (data.addresses || []);
                console.log("Backend response data:", data);
                setAddresses(addressArray.map((addr: AddressItem) => ({
                    ...addr,
                    isEditing: false
                })));
            } catch (error) {
    ...
    // 处理输入变化
    const handleChange = (id: number, field: keyof AddressItem, value: string) => {
    ...
    const handleSave = async (addr: AddressItem) => {
        ...
        const url = isNew ? `${BACKEND_URL}/addresses/addAddress` : `${BACKEND_URL}/addresses/updateAddress`;
    ...
    if (loading) return (<div className="flex h-screen items-center justify-center">Loading addresses...</div>);
            ...
            <div className="mx-auto max-w-3xl px-4 py-10">
                ...
                <div className="flex justify-between items-end mb-6">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Addresses Saved: {addresses.length} / Max: 5</p>
                    </div>
                            ...
                            <div key={addr.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 transition-all">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <Input
                                            ...
                                            readOnly={!addr.isEditing}
                                            onChange={(e) => handleChange(addr.id, 'recipient_name', e.target.value)}
                                            className={addr.isEditing ? "" : "bg-gray-100"}
                                        />
                                    </div>
                                    {/* 电话 */}
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(v) => handleChange(addr.id, 'country_code', v)}
                                        >
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <Input
                                            value={addr.zip_code}
                                            readOnly={!addr.isEditing}
                                            onChange={(e) => handleChange(addr.id, 'zip_code', e.target.value)}
                                            className={addr.isEditing ? "" : "bg-gray-100"}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="mt-4">
                                    ...
                                    <Textarea
                                        ...
                                        readOnly={!addr.isEditing}
                                        onChange={(e) => handleChange(addr.id, 'address_line', e.target.value)}
                                        rows={2}
